// End-to-end smoke test for Module 3.2 (Host-Resident Pool) + Module 3.3
// (GPU-Expert Cache), naive stall-and-copy baseline (dev_spec.md Section 8,
// step 3). Registers a few fake experts, deliberately undersizes the GPU
// cache so eviction has to happen, and times each access to show hits
...
int main() {
    ...
    for (uint32_t id = 0; id < kNumExperts; ++id) {
        ...
        if (!pool.register_expert(id, data, kExpertBytes)) {
            std::fprintf(stderr, "failed to register expert %u\n", id);
            return 1;
        }
    }
    ...
    std::printf("\ntotals: %zu hits, %zu misses (expected 1 hit, 4 misses)\n",
